...
public class MappingProfile : Profile
{
    public MappingProfile()
    {
        ...
        CreateMap<course, CourseDetailResponse>()
            ...
            .ForMember(dest => dest.Lecturers, opt => opt.MapFrom(src => src.lecturer_users != null ? src.lecturer_users : Enumerable.Empty<lecturer>()))
            .ForMember(dest => dest.Enrollments, opt => opt.MapFrom(src => src.course_enrollments != null ? src.course_enrollments.Where(e => e.status == "ACTIVE") : Enumerable.Empty<course_enrollment>()))
        ...
        CreateMap<project, ProjectDetailResponse>()
            ...
            .ForMember(dest => dest.GithubStatus, opt => opt.MapFrom(src => src.project_integration != null ? src.project_integration.approval_status : "NONE"))
            .ForMember(dest => dest.JiraStatus, opt => opt.MapFrom(src => src.project_integration != null ? src.project_integration.approval_status : "NONE"))
            .ForMember(dest => dest.CreatedAt, opt => opt.MapFrom(src => src.created_at))
            ...
            .AfterMap((src, dest) =>
            {
                if (src.project_integration != null)
                    dest.Integration = new JiraGithubExport.Shared.Contracts.Responses.Projects.IntegrationInfo
                    {
                        ...
                        JiraStatus = src.project_integration.approval_status ?? "PENDING",
