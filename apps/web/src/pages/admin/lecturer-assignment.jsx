...
export default function LecturerAssignment() {
  ...
  const handleRemove = async (courseId, lecturerId, lecturerName) => {
    if (!window.confirm(`Xóa giảng viên ${lecturerName} khỏi lớp này?`)) return;
    try {
      await removeMutation.mutateAsync({
        courseId,
        lecturerUserId: lecturerId
      });
      success("Đã gỡ bỏ phân công");
    } catch (err) {
  ...
  if (loadingCourses || loadingLects || loadingSems || loadingSubs) return (
     <div className="flex h-screen items-center justify-center p-8 bg-gray-50/50">
        <div className="text-center">
          ...
          <span className="text-gray-500 font-bold uppercase tracking-widest text-xs">Đang đồng bộ dữ liệu giảng viên...</span>
        </div>
     </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <PageHeader 
        ...
        actions={[
          <Button key="add" className="bg-teal-600 hover:bg-teal-700 text-white rounded-2xl h-11 px-6 text-[10px] font-black uppercase tracking-widest border-0 shadow-lg shadow-teal-100 transition-all" onClick={() => { setSelectedCourse(null); setModalOpen(true); }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         <StatsCard label="Tổng số lớp" value={courses.length} icon={Layers3} variant="info" />
      ...
      <Card className="border border-gray-100 shadow-sm rounded-[24px] overflow-hidden bg-white">
        <CardContent className="p-4 flex flex-col md:flex-row gap-4">
           <div className="flex-1">
              <InputField placeholder="Tìm kiếm mã lớp, tên lớp..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} icon={Search} />
           </div>
           <div className="flex gap-4">
              ...
              <Button variant="outline" className="rounded-xl border-gray-100 h-11 px-4 text-[10px] font-black uppercase tracking-widest shadow-sm" onClick={() => setSort(s => s === "students" ? "code" : "students")}>
                <ArrowUpDown size={14} className="mr-2" /> Sort {sort || "Mặc định"}
              </Button>
           </div>
        </CardContent>
      </Card>
               ...
               {paginated.map(c => (
                 <div key={c.id} className="p-8 hover:bg-teal-50/10 transition-all flex flex-col md:flex-row md:items-center justify-between gap-8 group">
                    <div className="flex-1 min-w-0">
                       <h3 className="font-black text-teal-700 text-lg uppercase tracking-tight">{c.code || c.course_code}</h3>
                       <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">{c.name || c.course_name}</p>
                    </div>
                    <div className="w-48">
                       <p className="text-[10px] font-black text-gray-300 uppercase mb-1">Học kỳ / Môn học</p>
                       <p className="text-xs font-bold text-gray-800">{semesters.find(s=>s.id===c.semesterId)?.name || c.semester_name || '—'}</p>
                       <p className="text-[10px] text-blue-500 font-bold mt-0.5">{subjects.find(s=>s.id===c.subjectId)?.code || c.subject_code || 'N/A'}</p>
                    </div>
                    <div className="flex-[1.5]">
                       <p className="text-[10px] font-black text-gray-300 uppercase mb-3">Giảng viên phụ trách</p>
                       <div className="flex flex-wrap gap-3">
                          {c.lecturers?.length ? c.lecturers.map(l => (
                            <div key={l.id} className="flex items-center gap-2 bg-white border border-teal-100 px-3 py-1.5 rounded-2xl text-teal-700 shadow-sm group/item">
                               <div className="w-6 h-6 rounded-full bg-teal-600 text-white flex items-center justify-center text-[10px] font-black shadow-inner uppercase">{l.name.charAt(0)}</div>
                          ...
                          )) : <span className="text-[11px] font-bold text-gray-300 italic tracking-wide">Chưa có giảng viên điều phối</span>}
                       </div>
                    </div>
                    <Button size="sm" variant="outline" className="rounded-xl h-11 px-6 text-[10px] font-black uppercase tracking-widest transition-all hover:bg-teal-600 hover:text-white border-gray-100 hover:border-teal-600 shadow-sm" onClick={() => { setSelectedCourse(c); setModalOpen(true); }}><Plus size={14} className="mr-2"/> Phân công</Button>
                 </div>
               ))}
               {paginated.length === 0 && (
                 <div className="py-20 text-center">
                    <BookOpen size={48} className="text-gray-200 mx-auto mb-4" />
                    <p className="text-xs font-black text-gray-300 uppercase tracking-widest">Không có lớp học phù hợp</p>
                 </div>
               )}
            </div>
         </CardContent>
         {totalPages > 1 && (
            <div className="p-6 border-t border-gray-50 flex items-center justify-between">
               ...
               <div className="flex gap-2">
                  <Button variant="ghost" disabled={page === 1} onClick={() => setPage(p => p - 1)} className="rounded-xl h-9 px-4 text-[10px] font-black uppercase border border-gray-100">Trước</Button>
                  <Button variant="ghost" disabled={page === totalPages} onClick={() => setPage(p => p + 1)} className="rounded-xl h-9 px-4 text-[10px] font-black uppercase border border-gray-100">Sau</Button>
               </div>
            </div>
         )}
      </Card>
           ...
           <Card className="w-[480px] rounded-[32px] border-0 shadow-2xl overflow-hidden bg-white animate-in zoom-in duration-300">
              <CardHeader className="bg-teal-600 text-white p-10">
                 <div className="flex justify-between items-center mb-6">
                    <CardTitle className="text-2xl font-black uppercase tracking-widest">Phân công Giảng viên</CardTitle>
                 ...
                 <div className="p-5 bg-white/10 rounded-2xl border border-white/20">
                    <p className="text-teal-50 text-[10px] font-black uppercase tracking-widest mb-1">Đang chọn lớp học</p>
                    <p className="text-white text-base font-black tracking-tight">{selectedCourse ? `${selectedCourse.code || selectedCourse.course_code} — ${selectedCourse.name || selectedCourse.course_name}` : 'Vui lòng chọn lớp học phía dưới'}</p>
                 </div>
              </CardHeader>
              <CardContent className="p-10 space-y-8">
                 {!selectedCourse && (
                   <div>
                      <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3 pl-1">Chọn lớp học phần</p>
                      <SelectField value={selectedCourse?.id || ''} onChange={e => setSelectedCourse(courses.find(c=>c.id===e.target.value))}>
                         <option value="">-- Danh sách các lớp học --</option>
                         {courses.map(c => <option key={c.id} value={c.id}>{c.code || c.course_code} - {c.name || c.course_name}</option>)}
                      </SelectField>
                   </div>
                 )}
                 <div>
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3 pl-1 block">Chọn Giảng viên chuyên trách</label>
                    <SelectField value={selectedLecturer} onChange={e => setSelectedLecturer(e.target.value)}>
                       ...
                       {lecturers.map(l => (
                         <option key={l.id} value={l.id}>
                           {l.name} (Đang gánh {lecturerWorkload[l.id] || 0} lớp)
                         </option>
                       ))}
                    </SelectField>
                    <p className="text-[10px] text-gray-400 mt-2 italic px-1">* Hệ thống sẽ phân tích workload để đưa ra gợi ý tốt nhất</p>
                 </div>
                 <div className="pt-4">
                    <Button 
                      className="w-full h-16 bg-teal-600 hover:bg-teal-700 text-white rounded-2xl font-black uppercase tracking-widest shadow-2xl shadow-teal-100 border-0 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50" 
                    ...
                    <Button variant="ghost" className="w-full mt-4 h-12 text-gray-400 font-bold uppercase tracking-widest text-[10px] hover:text-gray-600" onClick={() => setModalOpen(false)}>Hủy bỏ</Button>
                 </div>
              </CardContent>
           </Card>
        </div>
