...
export default function Performance() {
  ...
  const hasSummary = summary && (summary.productivity > 0 || summary.efficiency > 0 || summary.quality > 0 || summary.engagement > 0);
  const hasMembers = members?.rows?.length > 0;
  const hasTrends = trends?.buckets?.length > 0;
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      {/* Team Metrics (Hidden if empty placeholder) */}
      {hasSummary && (
      ...
      {/* Performance Charts */}
      {(hasMembers || hasTrends || membersLoading || trendsLoading) && (
              ...
              <CardContent className="p-4 lg:p-6">
                <div className="space-y-6">
                  {members.rows.map((row) => {
                    ...
                    return (
                      <div key={row.memberId || row.id}>
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium text-blue-900 text-sm lg:text-base">
                            {row.memberName || row.name || "Unknown"}
                          </span>
                          <span className="text-xs lg:text-sm font-semibold text-blue-600">{row.score || 0}%</span>
                        </div>
                        ...
                        <div className="flex gap-4 mt-2 text-xs text-blue-600">
                          <span>{row.commits || 0} commits</span>
                          <span>{row.issues || 0} issues</span>
                          <span>{row.tasks || 0} tasks</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs lg:text-sm">
                ...
                <tbody>
                  {members.rows.map((row) => {
                    return (
                      <tr key={row.memberId || row.id} className="border-t border-blue-100 hover:bg-blue-50/30 transition-colors">
                        <td className="px-4 lg:px-6 py-4 font-medium text-blue-900">{row.memberName || row.name || "Unknown"}</td>
                        <td className="px-4 lg:px-6 py-4">
                          <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">
                            {row.score || 0}%
                          </span>
                        </td>
                        <td className="px-4 lg:px-6 py-4 text-blue-600 hidden md:table-cell">{row.commits || 0}</td>
                        <td className="px-4 lg:px-6 py-4 text-blue-600 hidden md:table-cell">{row.issues || 0}</td>
                        <td className="px-4 lg:px-6 py-4 text-blue-600 hidden md:table-cell">{row.tasks || 0}</td>
                        <td className="px-4 lg:px-6 py-4 font-semibold text-blue-900">{row.total || 0}</td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      {!hasSummary && !hasMembers && !hasTrends && !summaryLoading && (
        <div className="text-center py-10 text-gray-500 italic">
          Các chỉ số Performance hiện chưa được hỗ trợ từ API. Vui lòng quay lại sau.
        </div>
      )}
    </div>
  );
