...
export default function Reports() {
 ...
 <h3 className="font-black text-gray-800 text-sm tracking-tight mb-2">{et.title}</h3>
 <p className="text-[11px] text-gray-400 font-medium leading-relaxed">{et.desc}</p>
 </div>
 <div className="mt-6 flex gap-2">
 {et.formats.map(f => (
 <button key={f} onClick={(e) => { e.stopPropagation(); handleExport(f, et.id, et.title); }} className="flex-1 py-1.5 rounded-lg bg-white border border-gray-100 text-[9px] font-black text-gray-400 hover:text-teal-600 hover:border-teal-200 transition-all">{f}</button>
 ))}
 </div>
 </div>
 ))}
