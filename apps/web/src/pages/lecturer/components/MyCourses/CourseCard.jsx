      ...
      <div className="relative bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden h-full">
        ...
        <CardContent className="p-8 pt-0 -mt-6 space-y-6">
          <div className="flex items-start justify-between">
            ...
            {alerts > 0 && (
              <div className="mt-4 flex items-center gap-1 px-3 py-1 bg-red-50 text-red-600 rounded-full border border-red-100 animate-pulse">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">{alerts} Cảnh báo</span>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{course.name || course.subject?.name || course.subjectName}</p>
            <div className="flex items-center gap-2 mt-4 text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50 px-3 py-1 rounded-full w-fit">
               Semester {semester}
            </div>
          </div>
          ...
          <div className="space-y-3 pt-2">
             <div className="flex justify-between items-end">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Tiến độ tích hợp</span>
          ...
          <div className="pt-4 flex flex-col gap-3">
             <div className="flex items-center justify-between p-4 rounded-2xl bg-gray-50/50 border border-gray-100 group-hover:bg-white group-hover:border-teal-100 transition-all">
                <div className="flex items-center gap-3">
                   ...
                   <div>
                      <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">GitHub Repos</p>
                      <p className="text-xs font-black text-gray-800">{activeTeams}/{groupCount}</p>
                   </div>
                </div>
                <div className="text-right">
                   <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Cuối cùng</p>
...
function StatusBadge({ status }) {
  const map = {
    ...
    LOW: "bg-amber-400/90 text-white border-white/20",
    "NO REPO": "bg-red-500/90 text-white border-white/20",
    ARCHIVED: "bg-gray-400/90 text-white border-white/20"
  };
  return (
    <span className={`text-[10px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-xl backdrop-blur-md border shadow-lg ${map[status] || map.ACTIVE}`}>
      {status}
    </span>
  );
}
