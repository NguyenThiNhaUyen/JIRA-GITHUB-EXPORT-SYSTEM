        ...
        <div className="bg-white rounded-[32px] border border-gray-50 h-full">
          <CardHeader className="border-b border-gray-50 py-8 px-10">
            <div className="flex items-center gap-3">
               ...
               <CardTitle className="font-display">Tạo Nhóm Thủ Công</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="p-10 space-y-8">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">Đề tài dự án</label>
              <InputField 
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center px-1">
                 <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Chọn sinh viên</label>
                 <span className="text-[10px] font-black text-teal-600 bg-teal-50 px-2 py-1 rounded-lg border border-teal-100 uppercase tracking-widest">{selectedStudents.length} Đã chọn</span>
              </div>
              <div className="max-h-72 overflow-y-auto border border-gray-100 rounded-[24px] divide-y divide-gray-50 bg-gray-50/20 custom-scrollbar p-2">
                {availableStudents.length === 0 ? (
                  <div className="p-12 text-center bg-white rounded-[18px]">
                     ...
                     <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest leading-relaxed">Tất cả sinh viên lớp học<br/>đã được phân bổ vào các nhóm</p>
                  </div>
                  ...
                  <label key={s.id} className={`p-4 flex items-center gap-4 rounded-[18px] cursor-pointer transition-all duration-300 group ${selectedStudents.includes(s.id) ? 'bg-teal-50 border-teal-100' : 'hover:bg-white hover:shadow-md'}`}>
                    <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${selectedStudents.includes(s.id) ? 'bg-teal-600 border-teal-600 text-white' : 'border-gray-200 bg-white group-hover:border-teal-300'}`}>
                       {selectedStudents.includes(s.id) && <span className="text-[10px] font-black">✓</span>}
                    </div>
                    <input
                      type="checkbox"
                    ...
                    <div className="min-w-0">
                      <p className={`text-xs font-black uppercase tracking-tight transition-colors ${selectedStudents.includes(s.id) ? 'text-teal-700' : 'text-gray-800'}`}>{s.name || s.fullName}</p>
                      <p className={`text-[9px] font-bold uppercase tracking-widest mt-0.5 transition-colors ${selectedStudents.includes(s.id) ? 'text-teal-600/60' : 'text-gray-400'}`}>{s.studentCode || s.id}</p>
                    </div>
                  </label>
                ))}
              </div>
            ...
            <Button
              className="w-full h-15 bg-gradient-to-br from-teal-500 to-teal-700 hover:from-teal-600 hover:to-teal-800 text-white rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl shadow-teal-100 transition-all active:scale-95 disabled:opacity-50 disabled:grayscale border-0 mt-4 h-14"
      ...
      <Card className="relative overflow-hidden group border-0 shadow-premium p-1.5 bg-gradient-to-br from-violet-600 to-indigo-700 rounded-[40px]">
        ...
        <div className="relative bg-white/95 backdrop-blur-sm rounded-[34px] p-8 space-y-6">
          ...
          <div className="bg-violet-50/50 p-4 rounded-2xl border border-violet-100 flex gap-3">
             ...
             <p className="text-[10px] text-violet-700 font-black leading-relaxed uppercase tracking-widest opacity-80">
                Tự động tối ưu hóa việc phân bổ {availableStudents.length} sinh viên còn lại dựa trên thuật toán cân bằng quy mô nhóm.
             </p>
          </div>

          <div className="space-y-3">
             <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">Cấu hình Quy mô Nhóm</label>
             <SelectField 
               value={autoGroupSize} 
               onChange={e => setAutoGroupSize(Number(e.target.value))} 
               className="h-12 border-violet-100 bg-white text-[10px] font-black uppercase tracking-widest rounded-xl focus:ring-violet-500 focus:border-violet-500"
             >
               <option value={4}>Standard: 4 SV / Nhóm</option>
               <option value={5}>Balanced: 5 SV / Nhóm</option>
               <option value={6}>Maximum: 6 SV / Nhóm</option>
             </SelectField>
          </div>

          <Button
            className="w-full h-14 bg-gradient-to-br from-violet-600 to-indigo-700 hover:from-violet-700 hover:to-indigo-800 text-white rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl shadow-indigo-100 transition-all active:scale-95 disabled:opacity-50 border-0"
            onClick={handleAutoCreateGroups}
            disabled={isBusy || availableStudents.length === 0}
          >
            <Wand2 size={18} className="mr-2" /> Bắt đầu thuật toán
          </Button>
        </div>
      </Card>
    </div>
