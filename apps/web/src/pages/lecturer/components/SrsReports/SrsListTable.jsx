...
export function SrsListTable({ 
 ...
 <p className="text-sm font-black text-gray-800 tracking-tight truncate">{item.teamName}</p>
 <p className="text-[10px] text-gray-400 font-bold mt-1 truncate max-w-[200px]">{item.projectName}</p>
 </div>
 </div>
 </td>
 <td className="py-6 px-8 text-center"><span className="text-[11px] font-black text-indigo-500 bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100">V{item.version || '1.0'}</span></td>
 <td className="py-6 px-8 text-center">
 <Badge variant="outline" className={`px-3 py-1 rounded-full font-black text-[9px] border transition-all ${STATUS_META[item.status]?.color || ''}`}>{STATUS_META[item.status]?.label}</Badge>
 </td>
 <td className="py-6 px-8 text-center">
