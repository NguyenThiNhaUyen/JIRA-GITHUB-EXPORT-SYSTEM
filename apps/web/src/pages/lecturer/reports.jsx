                ...
                {EXPORT_TYPES.map(et => (
                    ...
                    >
                        ...
                        <div className="mt-6 flex gap-2">
                             {et.formats.map(f => (
                                <button key={f} onClick={(e) => { e.stopPropagation(); success(`Xuất ${f} thành công`); }} className="flex-1 py-1.5 rounded-lg bg-white border border-gray-100 text-[9px] font-black uppercase tracking-widest text-gray-400 hover:text-teal-600 hover:border-teal-200 transition-all">{f}</button>
                             ))}
                        </div>
                    </div>
                ))}
                ...
                <Card className="lg:col-span-8 border border-gray-100 shadow-sm rounded-[32px] overflow-hidden bg-white">
                    <CardHeader className="border-b border-gray-50 py-6 px-8 flex flex-row items-center justify-between">
                         ...
                         <div className="flex gap-2">
                             {selectedConfig?.formats.map(fmt => <Button key={fmt} variant="outline" size="sm" className="rounded-xl border-gray-100 text-[10px] font-black uppercase tracking-widest h-9" onClick={() => success(`Download ${fmt}`)}><Download size={14} className="mr-2" />{fmt}</Button>)}
                         </div>
                    </CardHeader>
                ...
                <div className="lg:col-span-4 space-y-8">
                            ...
                            {MOCK_EXPORTS.map(ex => (
                                <div key={ex.id} className="p-5 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <div className="mt-3 flex gap-2">
                                        <Button variant="ghost" className="h-8 rounded-lg text-teal-600 bg-teal-50 hover:bg-teal-100 text-[10px] font-black uppercase tracking-widest px-4 border-0">Tải lại</Button>
                                    </div>
                                </div>
                            ))}
                        </CardContent>
                    ...
                    <Card className="border border-gray-100 shadow-sm rounded-[32px] overflow-hidden bg-white p-8">
                        <div className="text-center">
                            ...
                            <Button className="w-full h-12 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-indigo-100 border-0">Sync Now</Button>
                        </div>
                    </Card>
                </div>
            </div>
