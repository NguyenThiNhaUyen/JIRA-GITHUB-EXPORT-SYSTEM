        ...
        <Card className="rounded-[56px] border border-gray-100 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)] bg-white overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 glass-card" style={{ animationDelay: `${index * 200}ms` }}>
            <CardHeader className="p-12 border-b border-gray-50 flex flex-row items-center justify-between bg-gray-50/20">
                <div className="space-y-3">
                    <CardTitle className="text-xl font-black uppercase tracking-tighter text-gray-800 leading-none font-display mb-1">{project.name}</CardTitle>
                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em] leading-none flex items-center gap-3">
                        <div className="w-1.5 h-1.5 rounded-full bg-teal-500 animate-pulse" />
                        Nhóm hồ sơ SRS hiện tại
                    </p>
                </div>
                <div className="flex items-center gap-4">
                    <div className="bg-white px-6 py-3 rounded-[20px] flex items-center gap-3 border border-gray-100 shadow-sm transition-all group-hover:scale-105">
                        ...
                        <span className="text-[11px] font-black text-teal-700 uppercase tracking-widest">{srsList.length} PHIÊN BẢN</span>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="p-0 divide-y-2 divide-dashed divide-gray-50">
                {srsList.length === 0 ? (
                    <div className="p-24 text-center space-y-6">
                        ...
                        <p className="text-[11px] font-black text-gray-300 uppercase tracking-[0.3em] italic opacity-60">Dự án chưa có bản nộp đặc tả nào</p>
                    </div>
                ) : srsList.map((rpt, idx) => (
                    <div key={rpt.id} className="p-10 flex items-center justify-between hover:bg-teal-50/20 transition-all duration-500 group relative overflow-hidden">
                        ...
                        <div className="flex gap-10 items-center">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center gap-6">
                                    <p className="text-lg font-black text-gray-800 uppercase tracking-tight font-display">Version {rpt.version || (idx + 1).toFixed(1)}</p>
                                    <StatusBadge status={rpt.status === 'FINAL' || rpt.status === 'APPROVED' ? 'success' : 'warning'} label={rpt.status === 'FINAL' || rpt.status === 'APPROVED' ? 'Đã duyệt' : 'Đang xử lý'} />
                                </div>
                                <div className="flex items-center gap-3 text-[10px] font-black text-gray-300 uppercase tracking-widest bg-gray-50/50 px-3 py-1 rounded-lg w-fit">
                                ...
                                {rpt.feedback && (
                                    <div className="mt-4 p-5 bg-indigo-50/50 rounded-[24px] max-w-lg border border-indigo-100/30 group-hover:bg-white transition-colors shadow-sm">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest opacity-80">Phản hồi từ Giảng viên:</p>
                                        </div>
                        ...
                        {rpt.fileUrl && (
                            <Button 
                                variant="outline" 
                                className="rounded-[24px] h-14 px-8 text-[11px] font-black uppercase tracking-widest border-slate-100 text-slate-400 hover:text-teal-600 hover:bg-white hover:border-teal-200 shadow-sm transition-all active:scale-95 group-hover:scale-105 font-display" 
                                onClick={() => window.open(rpt.fileUrl, '_blank')}
                            >
                                <FileDown size={18} className="mr-3"/> Tải tài liệu
                            </Button>
                        )}
                    </div>
                ))}
