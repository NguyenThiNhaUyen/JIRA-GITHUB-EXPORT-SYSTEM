...
export function WorkspaceHeader({ course, group, isLeader, onSync, isSyncing, onBack }) {
    ...
    return (
        <div className="space-y-6">
            <button 
                onClick={onBack} 
                className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] text-teal-600 hover:text-teal-700 transition-all group font-display"
            >
                        ...
                        <div className="flex gap-10 items-center">
                            ...
                            <div>
                                <div className="flex items-center gap-4 mb-4">
                                    <span className="text-[10px] font-black text-teal-600 bg-teal-50 px-4 py-1.5 rounded-full border border-teal-100 uppercase tracking-[0.2em] shadow-sm">{course.subject?.code || course.code}</span>
                                    {isLeader && (
                                        <span className="flex items-center gap-2 text-[10px] font-black px-4 py-1.5 rounded-full border bg-amber-50 text-amber-600 border-amber-100 uppercase tracking-[0.2em] shadow-sm animate-pulse">
                                            <Crown size={14} className="fill-amber-600/20" /> Team Leader
                                        </span>
                                    )}
                                </div>
                                <h3 className="text-3xl font-black text-gray-800 uppercase tracking-tighter leading-none mb-3 font-display">{group?.name || "Member Workspace"}</h3>
                                <div className="flex items-center gap-6">
                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] flex items-center gap-2.5">
                        ...
                        {group && (
                            <div className="relative group/sync">
                                <Button 
                                    onClick={onSync}
                                    disabled={isSyncing}
                                    className="rounded-[28px] h-16 border-2 border-gray-100 bg-white text-gray-800 text-[11px] font-black uppercase tracking-[0.3em] px-12 hover:bg-teal-50 hover:text-teal-700 hover:border-teal-200 transition-all shadow-xl shadow-teal-900/5 active:scale-95 font-display disabled:opacity-50"
                                >
