            ...
            <CardContent className="p-0">
                {isLoading ? (
                    ...
                    <div className="p-16 text-center space-y-6 bg-gray-50/20">
                        ...
                        <p className="text-[10px] text-gray-300 font-black uppercase tracking-widest italic opacity-60">Chưa có commit nào gần đây 🍃</p>
                    </div>
                ) : (
                        ...
                        {commitsList.map((c, idx) => (
                            <div key={idx} className="p-8 px-10 flex items-start gap-6 hover:bg-gray-50/50 transition-all duration-300 group cursor-pointer animate-in slide-in-from-right-4" style={{ animationDelay: `${idx * 100}ms` }}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-black text-gray-800 leading-tight truncate font-display group-hover:text-teal-600 transition-colors uppercase tracking-tight">{c.message || "Updated code baseline"}</p>
                                    <div className="flex items-center justify-between mt-3">
                                        <p className="text-[9px] text-teal-600 font-black uppercase tracking-widest bg-teal-50 px-2 py-0.5 rounded-md border border-teal-100">
                                            {c.repositoryName || c.repoName || "GitHub Repo"}
                                        </p>
                                        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest opacity-60">{new Date(c.commitDate || c.Date).toLocaleDateString("vi-VN")}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
