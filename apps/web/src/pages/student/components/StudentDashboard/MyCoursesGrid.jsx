...
export function MyCoursesGrid({ isLoading, courses, onCourseNavigate, onSeeAll }) {
 ...
 <CardTitle className="text-lg font-black text-gray-800 tracking-[0.2em] font-display">Lá»›p há»c cá»§a tĂ´i</CardTitle>
 <Button 
 variant="ghost" 
 className="text-[11px] font-black tracking-[0.2em] text-teal-600 hover:text-teal-700 bg-teal-50 px-6 h-12 rounded-[24px] border border-teal-100/50 transition-all font-display hover:scale-105"
 onClick={onSeeAll}
 >
 Xem táº¥t cáº£ <ChevronRight size={16} className="ml-2" />
 </Button>
 </CardHeader>
 ...
 <div className="w-16 h-16 rounded-[24px] bg-white flex items-center justify-center shadow-xl text-teal-600 font-black text-lg group-hover/item:bg-teal-600 group-hover/item:text-white group-hover/item:scale-110 group-hover/item:rotate-6 transition-all duration-500 font-display">
 {course.code?.substring(0, 3)}
 </div>
 <StatusBadge status={course.status} label={course.status} variant={course.status === 'ACTIVE' ? 'success' : 'default'} />
 </div>
 
 <p className="font-black text-gray-800 text-xl tracking-tighter leading-none mb-2 font-display">{course.code}</p>
 <p className="text-[11px] text-gray-400 font-black tracking-[0.2em] truncate mb-6 opacity-70 group-hover/item:text-teal-600 transition-colors">{course.name}</p>
 
 <div className="space-y-3">
 <div className="flex justify-between text-[9px] font-black text-gray-300">
 <span>Lá»›p: {course.currentStudents || 0} SV</span>
 ...
 <p className="text-[11px] font-black text-gray-300 tracking-[0.4em] leading-relaxed">Báº¡n chÆ°a Ä‘Äƒng kĂ½ tham gia khĂ³a há»c nĂ o</p>
 </div>
 )}
 </CardContent>
 </Card>
