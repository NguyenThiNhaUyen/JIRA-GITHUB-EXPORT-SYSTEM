...
export function UpcomingDeadlines({ isLoading, upcomingDeadlines, onSeeAll }) {
 ...
 <p className="text-sm font-black text-gray-800 leading-tight truncate font-display group-hover:text-red-700 transition-colors tracking-tight">{item.title}</p>
 <p className="text-[10px] text-gray-400 font-black tracking-[0.2em] mt-2 opacity-60 group-hover:text-red-400 transition-colors">{item.due || item.dueDate || item.deadline ||"TBA"}</p>
 </div>
