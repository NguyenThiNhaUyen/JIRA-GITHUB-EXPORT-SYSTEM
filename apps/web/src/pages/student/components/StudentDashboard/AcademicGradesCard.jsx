...
export function AcademicGradesCard({ isLoading, grades }) {
 ...
 <p className="text-sm font-black text-gray-800 tracking-tight font-display group-hover:text-orange-600 transition-colors truncate">{g.subjectName || g.Subject || g.category ||"Evaluation"}</p>
 <div className="flex items-center gap-3 mt-2">
 <p className="text-[9px] text-gray-400 font-black opacity-60">{g.projectName ||"General Study"}</p>
 <div className="w-1 h-1 rounded-full bg-gray-200" />
 <p className="text-[8px] text-orange-500 font-black bg-orange-50 px-2 py-0.5 rounded-md border border-orange-100/50">{g.weight ? `${g.weight}%` :"Final"}</p>
 </div>
 </div>
 ...
 <span className="text-[9px] font-black tracking-[0.2em] text-gray-300 opacity-60">SCORE INDEX</span>
 </div>
 </div>
 ))}
 </div>
