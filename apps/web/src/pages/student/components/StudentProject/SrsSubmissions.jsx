    ...
    return (
        <Card className="border border-gray-100 shadow-sm rounded-[44px] overflow-hidden bg-white">
            <CardHeader className="border-b border-gray-50/50 py-10 px-12 flex justify-between items-center bg-gray-50/10">
                <CardTitle className="text-xs font-black text-gray-400 uppercase tracking-widest font-display">Bản nộp tài liệu SRS (Software Requirements Specification)</CardTitle>
                <Button onClick={onUploadModalOpen} className="bg-teal-50 hover:bg-teal-100 text-teal-700 rounded-[20px] h-12 px-8 text-[11px] font-black uppercase tracking-widest border border-teal-100 transition-all font-display hover:scale-105 active:scale-95 shadow-sm">Submit New</Button>
            </CardHeader>
            <CardContent className="p-0">
                {loadingSrs ? (
                    <div className="py-32 text-center space-y-4">
                        <FileText className="mx-auto text-gray-100 animate-bounce" size={48}/>
                        <p className="text-[11px] font-black text-gray-300 uppercase tracking-widest">Đang tải tài liệu...</p>
                    </div>
                ) : srsReports.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-36 gap-8 text-center bg-gray-50/5">
                        ...
                        <p className="text-[11px] font-black text-gray-400 uppercase tracking-[0.3em] leading-relaxed">Chưa có tài liệu nào được nộp.<br/>Phân loại SRS là bắt buộc để giảng viên duyệt.</p>
                    </div>
                ) : (
                        ...
                        {srsReports.map((rpt, idx) => (
                            <div key={rpt.id} className="p-10 px-12 flex items-center justify-between hover:bg-gray-50/50 group transition-all animate-in slide-in-from-bottom-2 duration-500" style={{ animationDelay: `${idx * 150}ms` }}>
                                <div className="flex items-center gap-10">
                                    ...
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-4 mb-3">
                                            <p className="text-base font-black text-gray-800 uppercase tracking-tight font-display">{rpt.fileName || `SRS_Version_${rpt.version}.pdf`}</p>
                                            <StatusBadge status={rpt.status === 'FINAL' ? 'success' : 'warning'} label={rpt.status} variant={rpt.status === 'FINAL' ? 'success' : 'warning'} />
                                        </div>
                                        <p className="text-[11px] text-gray-400 font-bold uppercase tracking-widest bg-white border border-gray-100 px-3 py-1 rounded-xl shadow-sm inline-flex items-center gap-2">
                                        ...
                                        {rpt.feedback && (
                                            <div className="mt-5 bg-indigo-50/30 border border-indigo-100/50 p-6 rounded-[24px] max-w-lg relative overflow-hidden group/feedback">
                                                ...
                                                <p className="text-[11px] font-black text-indigo-400 uppercase tracking-widest mb-2 opacity-60">Phản hồi từ giảng viên</p>
                                ...
                                <div className="flex items-center gap-6 shrink-0">
                                    <Button 
                                        variant="outline" 
                                        className="rounded-[24px] h-14 px-10 text-[11px] font-black uppercase tracking-widest border-gray-100 bg-white hover:bg-teal-50 hover:text-teal-700 hover:border-teal-200 hover:shadow-2xl transition-all font-display shadow-sm active:scale-95"
