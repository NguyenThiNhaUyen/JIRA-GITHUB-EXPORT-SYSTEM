...
export function ProjectSummarySidebar({ loadingProject, project, onSync, isSyncing, roadmapData }) {
 ...
 <CardTitle className="text-[11px] font-black text-gray-400 mb-10 flex items-center gap-3">
 ...
 <div>
 <h3 className="text-3xl font-black text-gray-800 tracking-tighter leading-tight mb-8 font-display group-hover:text-teal-600 transition-colors">
 ...
 onClick={onSync} 
 disabled={isSyncing} 
 className="w-full bg-slate-900 hover:bg-black text-white rounded-[28px] h-20 font-black shadow-2xl shadow-slate-200 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 border-0 group/sync"
 >
 {isSyncing ? (
 ...
 <p className="text-sm font-black text-gray-800 tracking-tight truncate grow group-hover:text-teal-600 transition-colors font-display pr-2">{item.title}</p>
 <div className={`text-[8px] font-black px-3 py-1 rounded-full border ${item.status?.toUpperCase() === 'DONE' ? 'text-teal-500 border-teal-100 bg-teal-50/50' : 'text-indigo-500 border-indigo-100 bg-indigo-50/50'} opacity-80 transition-all shadow-sm`}>{item.status}</div>
 </div>
 ...
 <p className="text-sm font-black text-gray-800 tracking-tight group-hover:text-teal-600 transition-colors font-display pr-2">{d.title}</p>
 <div className={`text-[8px] font-black px-3 py-1 rounded-full border ${d.color.replace('text-', 'border-')} bg-white shadow-sm opacity-80`}>{d.status}</div>
 </div>
 <p className="text-[10px] text-gray-400 font-bold">{d.due}</p>
 </div>
 </div>
 ))
 )}
