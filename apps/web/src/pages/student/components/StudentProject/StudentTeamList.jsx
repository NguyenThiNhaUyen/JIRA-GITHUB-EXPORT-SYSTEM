    ...
    return (
        <Card className="border border-gray-100 shadow-sm rounded-[40px] overflow-hidden bg-white">
            <CardHeader className="border-b border-gray-50 py-10 px-12 bg-gray-50/10">
                <CardTitle className="text-xs font-black text-gray-400 uppercase tracking-widest font-display">Danh sách thành viên & Chỉ số đóng góp</CardTitle>
            </CardHeader>
                        ...
                        <div key={m.studentId} className="flex items-center gap-10 px-12 py-10 border-b border-gray-50 last:border-0 hover:bg-gray-50/40 transition-all group animate-in slide-in-from-bottom-2 duration-500" style={{ animationDelay: `${idx * 100}ms` }}>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-4 mb-4">
                                    <p className="text-lg font-black text-gray-800 tracking-tight uppercase font-display">{m.studentName}</p>
                                    <div className="flex gap-2">
                                        {isLead && <Badge variant="outline" className="bg-amber-50 border-amber-200 text-amber-700 text-[9px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full shadow-sm">⭐ Leader</Badge>}
                                        {isMe && <Badge variant="outline" className="bg-teal-50 border-teal-200 text-teal-700 text-[9px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full shadow-sm">Bạn</Badge>}
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-8">
                                    ...
                                    <span className="text-[11px] font-black text-gray-700 uppercase tracking-widest shrink-0 w-32 text-right">{m.contributionScore || 0}% Đóng góp</span>
                                </div>
                            </div>
                            
                            <div className="hidden md:flex gap-12 shrink-0">
                                <div className="text-center group/metric">
                                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-2 opacity-60 group-hover/metric:text-teal-500 transition-colors">Commits</p>
                                    <p className="font-black text-3xl text-teal-600 tracking-tighter leading-none">{studentMetric?.commits || 0}</p>
                                </div>
                                <div className="text-center group/metric">
                                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-2 opacity-60 group-hover/metric:text-indigo-400 transition-colors">Issues</p>
