    ...
    <Card className="border border-gray-100 shadow-sm rounded-[24px] overflow-hidden bg-white">
      <CardHeader className="border-b border-gray-50 pb-4 px-6">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-base font-black text-gray-800">Xáº¿p háº¡ng Ä‘Ă³ng gĂ³p sinh viĂªn</CardTitle>
        </div>
      </CardHeader>
      ...
      <div className="hidden xl:grid grid-cols-12 gap-2 px-6 py-3 bg-gray-50/50 text-[10px] font-bold text-gray-400 border-b border-gray-100">
        ...
        <div className="col-span-3">Sinh viĂªn / NhĂ³m</div>
        ...
        <div className="col-span-2 text-right">Thao tĂ¡c</div>
      </div>

      <CardContent className="p-0">
        {students.length === 0 ? (
          <div className="py-20 text-center text-gray-400 italic text-sm">KhĂ´ng cĂ³ dá»¯ liá»‡u sinh viĂªn</div>
        ) : (
          students.map((s, i) => (
            <div key={s.studentId} className="grid grid-cols-12 gap-2 px-6 py-4 items-center border-b border-gray-50 hover:bg-teal-50/30 transition-all last:border-0">
              ...
              <div className="col-span-2 flex justify-end gap-1">
                <button onClick={() => onWarning(s)} className="p-1.5 rounded-lg border border-amber-100 text-amber-600 hover:bg-amber-50" title="Nháº¯c nhá»Ÿ">
                  Cáº§n chĂº Ă½
                </button>
              </div>
            </div>
