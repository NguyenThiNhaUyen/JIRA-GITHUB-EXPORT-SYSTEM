...
export function StudentContributionTable({ students, getStatusBadgeClass, onWarning, onEmail, sentMap, shouldWarnStudent }) {
  return (
    <Card className="border border-gray-100 shadow-sm rounded-[24px] overflow-hidden bg-white">
      ...
      <div className="hidden xl:grid grid-cols-12 gap-2 px-6 py-3 bg-gray-50/50 text-[10px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100">
          ...
          students.map((s, i) => (
            <div key={s.studentId} className="grid grid-cols-12 gap-2 px-6 py-4 items-center border-b border-gray-50 hover:bg-teal-50/30 transition-all last:border-0">
              ...
              <div className="col-span-1 text-center">
                <span className={`text-[9px] font-black px-2 py-0.5 rounded-full border uppercase tracking-widest ${getStatusBadgeClass(s.status)}`}>
                  {s.status}
                </span>
              </div>
