...
export function WeeklyActivityChart({ weeklyCommits }) {
  const counts = weeklyCommits.map(w => typeof w === 'object' ? w.count : w);
  const maxWeekly = Math.max(...counts, 1);
 ...
 <p className="text-[9px] font-black text-gray-400">Trung bình</p>
 <p className="text-sm font-black text-gray-700">{avg} commits/tuần</p>
 </div>
 <div className="p-3 rounded-2xl bg-gray-50 border border-gray-100">
 <p className="text-[9px] font-black text-gray-400">Tổng cộng</p>
