...
export function SrsTable({ items, selectedId, onSelect, statusMeta }) {
  const renderStatusChip = (status) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[9px] font-black ${meta.chip}`}
      >
  ...
  return (
    <Card className="rounded-[24px] border border-gray-100 shadow-sm bg-white overflow-hidden">
      <div className="grid grid-cols-12 gap-2 px-6 py-3.5 bg-gray-50/50 text-[10px] font-bold text-gray-400 border-b border-gray-100">
        <div className="col-span-5">NhĂ³m / Dá»± Ă¡n</div>
        <div className="col-span-2 text-center">Milestone</div>
        <div className="col-span-2 text-center">Tráº¡ng thĂ¡i</div>
        <div className="col-span-3 text-right">Cáº­p nháº­t</div>
      </div>
      <CardContent className="p-0 overflow-y-auto max-h-[600px] scrollbar-hide">
        {items.length === 0 ? (
          <div className="py-20 text-center text-gray-400 italic text-sm">
            KhĂ´ng tĂ¬m tháº¥y bĂ¡o cĂ¡o nĂ o phĂ¹ há»£p
          </div>
        ) : (
          items.map((item) => (
            <div
              ...
              className={`grid grid-cols-12 gap-2 px-6 py-4 items-center border-b border-gray-50 transition-all cursor-pointer last:border-0 hover:bg-teal-50/30 ${selectedId === item.id ? "bg-teal-50 border-l-4 border-l-teal-500 pl-5" : ""
                }`}
            >
