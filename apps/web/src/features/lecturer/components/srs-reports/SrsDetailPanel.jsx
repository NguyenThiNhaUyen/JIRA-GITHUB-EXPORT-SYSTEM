    ...
    return (
      <Card className="rounded-[24px] border border-dashed border-gray-200 shadow-none bg-gray-50/30 h-[600px] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 text-center max-w-[200px]">
          ...
          <div>
            <p className="text-sm font-bold text-gray-500">Chưa chọn báo cáo</p>
            <p className="text-xs text-gray-400 mt-1">Chọn một báo cáo từ danh sách để xem chi tiết và chấm điểm</p>
          </div>
        </div>
      </Card>
    );
    ...
    <Card className="rounded-[24px] border border-gray-100 shadow-xl bg-white sticky top-6 overflow-hidden animate-in slide-in-from-right duration-300">
      <CardHeader className="bg-gray-50/50 border-b border-gray-100 py-5 px-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-base font-black text-gray-800">Chi tiết SRS</CardTitle>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className={`w-2 h-2 rounded-full ${status.dot} animate-pulse`} />
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{status.label}</span>
            </div>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-0 overflow-y-auto max-h-[calc(100vh-200px)] custom-scrollbar">
        ...
        <div className="p-6 space-y-5">
          <div className="space-y-4">
            <div className="group">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Nhóm & Leader</label>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-500 font-medium">Leader: {selectedSrs.leader || "—"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-gray-50/80 border border-gray-100 space-y-3">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">Môn học</label>
                  <p className="text-[11px] font-bold text-gray-700">{selectedSrs.courseCode}</p>
                </div>
                <div>
                  <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">Milestone</label>
                  <p className="text-[11px] font-bold text-gray-700">{selectedSrs.milestone || "N/A"}</p>
                </div>
              </div>
              <div>
                <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">Dự án</label>
            ...
            <div>
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 block">Tài liệu đính kèm</label>
              <div className="flex gap-2">
                ...
                >
                  <ExternalLink size={14} className="mr-2" /> Xem trực tuyến
                </Button>
              </div>
            </div>
          </div>
        ...
        <div className="px-6 pb-6 pt-2 border-t border-gray-100 bg-gray-50/30">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="text-xs font-black text-gray-800 uppercase tracking-widest">Phản hồi của giảng viên</h4>
          </div>
          ...
          <div className="space-y-4">
            <div className="relative group">
              <textarea
                ...
                placeholder="Nhập nhận xét chi tiết cho nhóm (ví dụ: cần chỉnh sửa logic sơ đồ tuần tự...)"
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button
                disabled={isUpdating}
                className="rounded-xl h-12 bg-teal-600 hover:bg-teal-700 text-white border-0 shadow-lg shadow-teal-100 text-xs font-black uppercase tracking-wider"
              ...
              <Button
                disabled={isUpdating}
                variant="outline"
                className="rounded-xl h-12 bg-white border-red-100 text-red-600 hover:bg-red-50 text-xs font-black uppercase tracking-wider"
                onClick={() => onUpdateStatus(selectedSrs.id, 'NEED_REVISION')}
              >
                <Ban size={16} className="mr-2" /> Yêu Cầu Sửa
              </Button>
            </div>
            ...
            >
              Chỉ lưu nhận xét (không chuyển trạng thái)
            </Button>
          </div>
        </div>
        ...
        <div className="p-6 bg-gray-100/50 border-t border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-gray-400">
            <Clock size={12} />
            <span className="text-[10px] font-bold uppercase">Cập nhật lúc:</span>
          </div>
